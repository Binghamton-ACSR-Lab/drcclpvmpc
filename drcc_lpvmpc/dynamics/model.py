...
import numpy as np
from lpvdro.rkmath.rk6 import odeintRK6
from lpvdro.rkmath.rk6 import GeneralodeintRK6

# from include.euler1 import eulerint1
from lpvdro.rkmath.rk4 import odeintRK4
...
class KinematicModel:
	...
	def _integrate(self, x_t, u_t, t_start, t_end):
		"""	integrates using an RK6 ODE solver
			returns x_t at t_end = ∫ dxdt dt given x_t at t_start
			x_t is a 6x1 vector: [x, y, psi, v, steer, Delta]^T, Delta denotes the travel distance
		"""
		fun=self._diffequation
		odesol = odeintRK4(
			fun=fun, 
	...
	def plot_results(self, t, x, dxdt, u, friction_circle=False):
		...
		plt.plot(t[1:], u[0,:], label='velocity [m/s]')
		plt.plot(t[1:], u[1,:], label='steering')
		plt.ylabel('inputs')
		plt.grid(True)
		plt.legend()
