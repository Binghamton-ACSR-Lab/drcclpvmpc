...
def GeneralodeintRK6(fun, y0, t, args=()):
    gamma = np.asarray([7/90, 0, 32/90, 12/90, 32/90, 7/90])
    y_next = np.zeros([len(t)-1, len(y0)])
    ...
    for i in range(len(t)-1):
        ...
        k1 = h*fun(t[i], y0, *args)
        k2 = h*fun(t[i]+h/4, y0+k1/4, *args)
        k3 = h*fun(t[i]+1/4*h, y0+1/8*k1+1/2*k2, *args)
        k4 = h*fun(t[i]+1/2*h, y0-1/2*k2+k3, *args)
        k5 = h*fun(t[i]+3/4*h, y0+3/16*k1+9/16*k4, *args)
        k6 = h*fun(t[i]+h, y0-3/7*k1+2/7*k2+12/7*k3-12/7*k4+8/7*k5, *args)
        K = np.asarray([k1, k2, k3, k4, k5, k6])
        y_next[i,:] = y0 + gamma@K
        y0 = y0 + gamma@K
    return y_next